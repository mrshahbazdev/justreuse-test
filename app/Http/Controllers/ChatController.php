...
class PostDetailComponent extends Component
{
    ...
    public function submitOffer()
    {
        ...
        $this->validate([
            'offerPrice' => 'required|numeric|min:0|max:' . $this->post->price,
            'offerMessage' => 'nullable|string|max:500',
        ]);
        
        $this->showOfferModal = false;
        $this->reset(['offerPrice', 'offerMessage']);
        $this->dispatchBrowserEvent('show-toast', ['message' => 'Your offer has been sent.']);
    }
    ...
    private function prepareFeatures()
    {
        ...
        foreach ($post_values as $value) {
            ...
            if (in_array($field->type, ['select', 'autocomplete', 'radio-group'])) {
                 $option = TblFieldsOption::where('field_id', $field->id)->where('value', $post_value)->first();
                 $processed_value = $option ? $option->key : $post_value;
            } elseif ($field->form_field_name === 'brandwithmodel') {
                $values = explode(',', $post_value);
                $brand_name = TblFieldsOption::where('id', $values[0] ?? null)->value('key');
                $model_name = Str::title(str_replace('-', ' ', $values[1] ?? null));
