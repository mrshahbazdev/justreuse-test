<div class="flex justify-center" wire:ignore><div id="map_views"></div></div>

<?php
// $myip = $_SERVER['REMOTE_ADDR'];
$myip = "122.165.237.219";
...
function initMap() {
    ...
    $("#map_views").css('display', 'block');
    //$("#banner_map").css('height', '500px');
    $("#map_views").prop('class', 'w-full h-96');
    var curr_lat = jQuery("#lat").val();  //"9.9202912";
    // console.log('ndhgf');
    // console.log(jQuery("#lat").val());
    var curr_lon =   jQuery("#log").val();  // "78.1294314";
    ...
    // //end - get current location lat, lang fdgyyhth
    
    var center = new google.maps.LatLng(parseFloat(curr_lat), parseFloat(curr_lon));
    // MAP ATTRIBUTES.
    var mapAttr = {center: center, zoom: 12, mapTypeId: google.maps.MapTypeId.TERRAIN};
    //THE MAP TO DISPLAY.
    var map = new google.maps.Map(document.getElementById("map_views"), mapAttr);
...
//ajax
        ...
        $.ajax({
            ...
            success: function (data)
            {
                ...
                //THE MAP TO DISPLAY.
                var map = new google.maps.Map(document.getElementById("map_det"), mapAttr);
                //begin - circle shown for current city with specific km distance
                var circle = new google.maps.Circle({
                    center: center,
                    map: map,
