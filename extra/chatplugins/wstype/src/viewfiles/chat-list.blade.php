...
@section('content')
	...
	@endif
				...
				<div class="container mx-auto px-4">
					<div class="w-full block bg-gray-100 float-left border rounded-lg p-0 md:p-4 mb-5 md:flex justify-center">
						...
						<div class="user_chat_block hidden md:block w-full md:w-5/12 lg:w-2/5 bg-gray-50 float-left py-4 pb-0 rounded-lg overflow-auto mb-8 md:mb-0">
							...
							@else
							<div class="search_icon relative max-w-md mx-auto mb-2 px-2">
								<input type="text" wire:model="search" class="w-full bg-gray-50 pr-12 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 p-3" type="text" placeholder="{{__('messages.search by product name')}}" />
								...
								<div class="w-full px-3 bg-white float-left user_chats lg:px-5 xl:px-8 py-4 border-b relative {{ $selected }}">
									<a href="{{$chaturl}}" class="chat_lists w-full inline-block">
										...
										<p class="">
											...
											<span class="bg-green-400 px-1.5 py-0.5 pb-1 lg:px-1.5 lg:py-0.5 lg:pb-1 {{$class_dir_mr_lr}} leading-tight rounded-full text-sm text-white" data-ids="{{ $chatlist->post_id . '.' . $chatlist->receiver }}">
												{{$unread_count}}
											</span>
											...
											<button class="delete_all {{ $selected }} focus:outline-none bg-transparent absolute top-1/2 {{$class_dir_rl}}-3 px-2.5 py-1 text-red-500 text-xl font-bold" title="Delete Chat" data-id="{{ $sender . '@' . $chatlist->post_id }}"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
							...
							<div class="w-full float-left h-full relative">
								<div class="w-full bg-green-500 rounded-lg p-4 float-left">
									...
									<div class="w-3/12 {{$class_dir_float_rl}} pb-0 pt-2 lg:p-4 lg:pt-6 lg:w-3/12">
										...
										<span class="{{$class_dir_float_lr}} inline-block align-middle text-white text-xl lg:text-3xl"><a href="#"><i class="fa fa-map-marker" aria-hidden="true"></i></a></span>
									</div>
								</div>
								...
								<div class="w-full bg-white float-left">
									...
									<div class="w-full chat_converstion p-4 pb-36 md:pb-4" data-selected-ids="<?php echo ($check_blocked_user == 0) ? $receiver_detail . "." . request()->p : "" ?>" id="messages_area">
										<div class="show_bottom h-full overflow-y-scroll">
											...
											<div class="w-full mesage float-left">
												...
												<?php $make_offer_msg = ($detail->make_offer == 1)?"Your Offer ".$currency_symbol[0].$detail->msg:$detail->msg; ?>
								
												<div class="w-full {{$class_dir_float_rl}}">
													<div class="bg-gray-100 chat_message inline-block py-2 lg:py-4 px-5 my-4 rounded-3xl rounded-br-3xl rounded-tr-none {{$class_dir_float_rl}} {{$class_dir_mr_rl}}">
														@if(!empty($detail->attachment))
														<div id="chat_conv_image" data-img="{{ URL::to('/storage/'.$detail->attachment) }}" target="_blank">
															<div class="w-36 h-36 lg:w-44 lg:h-44 xl:w-56 xl:h-56 flex items-center justify-center"><img class="max-w-full max-h-full" src="{{ URL::to('/storage/'.$detail->attachment) }}" /></div>
														</div>
														<p><time class="text-xs text-gray-500 {{$class_dir_float_rl}} block {{$class_dir_text_rl}} w-36 ">{{ $time }} <span><?php echo $tick ?></span></time></p>
														@else
														@if($detail->accept_offer == 1)
														...
														<p class="text-base text-gray-800"><?php echo $make_offer_msg; ?></p>
														@endif
														<time class="text-xs text-gray-500 {{$class_dir_float_rl}} block {{$class_dir_text_rl}} w-36 ">{{ $time }} <span><?php echo $tick ?></span></time>
												...
												@else
												<?php $make_offer_msg = ($detail->make_offer == 1)?"Buyer Offer ".$currency_symbol[0].$detail->msg:$detail->msg; ?>
												<div class="w-full float-left">
												
													<div class="chat_message inline-block py-2 lg:py-4 px-5 my-4 rounded-3xl  rounded-tr-3xl rounded-br-none bg-green-500 {{$class_dir_float_lr}}">
														<div class="block">
															@if(!empty($detail->attachment))
															<div id="chat_conv_image" data-img="{{ URL::to('/storage/'.$detail->attachment) }}" target="_blank">
																<div class="w-36 h-36 lg:w-44 lg:h-44 xl:w-56 xl:h-56 flex items-center justify-center">
																	<img class="max-w-full max-h-full rounded m-auto" src="{{ URL::to('/storage/'.$detail->attachment) }}" />
																</div>
															</div>
															<p><time class="text-xs text-white {{$class_dir_float_rl}} block {{$class_dir_text_rl}} w-36 ">{{ $time }}</time></p>
															@else
															@if($detail->accept_offer == 1)
															...
															<p class="text-base text-white"><?php echo $make_offer_msg; ?></p>
															@endif
															<time class="text-xs text-white {{$class_dir_float_rl}} block {{$class_dir_text_rl}} w-36 ">{{ $time }}</time>
														</div>
												...
												?>
													...
													<div class="mb-2 w-full {{ $class_dir_float_rl }}">
													<div class="inline-block py-2 lg:py-4 px-5 my-4 rounded-3xl mr-2 rounded-br-3xl rounded-tr-none bg-gray-100 {{ $class_dir_float_rl }}">
													<p class="{{ $class_dir_float_rl }}">
														<a class="viewShared w-36 h-36 lg:w-44 lg:h-44 xl:w-56 xl:h-56 inline-block" href="https://www.google.com/maps?daddr={{$lat}},{{$long}}" target="_blank">
															<img class="h-full w-full object-cover object-center" src="https://maps.googleapis.com/maps/api/staticmap?center={{$lat}},{{$long}} ?>&amp;zoom=16&amp;size=400x200&amp;sensor=false&amp;maptype=roadmap&amp;markers=color:red%7Clabel:S%7C<?php echo $lat; ?>,<?php echo $long; ?>&amp;key=<?php echo $google_api_key;?>"></a>
													</p> 
													<p><time class="text-xs text-gray-500 {{$class_dir_float_rl}} block {{$class_dir_text_rl}} w-36 ">{{ $time }} 
									...
									<div class="w-full float-left px-4 xl:px-6 py-4 pb-0 absolute bottom-0 left-0 right-0 bg-white">
										...
										<div class="rounded w-full bg-white border-t border-gray-100">
											...
											<!-- Tab Contents -->
											<div id="tab-contents" class="h-44 overflow-y-scroll">
												<div id="first" class="my-3 px-2 rounded">
													...
													<ul class="list-unstyled inline-flex defalut-chat flex-wrap px-2">
														<li class="bg-white text-green-500 font-medium text-sm lg:text-base px-2 py-1 pb-2 lg:px-4 lg:py-2 cursor-pointer m-1 border-2 border-dashed border-green-500 rounded-lg lg:rounded-xl xl:rounded-2xl shadow-lg hover:text-white hover:bg-green-500 transition-all ease-linear duration-500" data-defmsg="hello">Hello</li>
														...
														<div class="make-offer-design">
															...
															<div class="w-full inline-block mb-4">
																<ul class="list-unstyled inline-flex flex-wrap">
																	<li row-val="5" class="bg-white text-green-500 font-medium text-sm lg:text-base px-2 py-1 pb-2 lg:px-6 xl:px-8 lg:py-2 lg:py-3 cursor-pointer mx-1 my-2 lg:m-2 border-2 border-dashed border-green-500 rounded-lg lg:rounded-xl xl:rounded-3xl shadow-lg hover:text-white hover:bg-green-500 transition-all ease-linear duration-500 price-active" data-defcurncy="<?php echo $currency_symbol[0]; ?>" data-defmsg="<?php echo floor($post_price); ?>"><?php echo $currency_symbol[0] . floor($post_price); ?></li>
																	<?php
																	$perc = 0;
																	$offer = "";
																	for ($j = 1; $j <= 4; $j++) {
																	...
																	?>
																	<li row-val="{{$j}}" class="bg-white text-green-500 font-medium text-sm lg:text-base px-2 py-1 pb-2 lg:px-6 xl:px-8 lg:py-2 lg:py-3 cursor-pointer mx-1 my-2 lg:m-2 border-2 border-dashed border-green-500 rounded-lg lg:rounded-xl xl:rounded-3xl shadow-lg hover:text-white hover:bg-green-500 transition-all ease-linear duration-500" data-defcurncy="<?php echo $currency_symbol[0]; ?>" data-defmsg="<?php echo $offer; ?>"><?php echo $currency_symbol[0] . $offer; ?></li>
																	<?php } ?>
																</ul>
															</div>
		...
		<script>
			$(document).ready(function() {
				var conn = new WebSocket("wss://127.0.0.1:6001/");
						...
						if (data.from == "Me") {
							if (data.msg != "") {
								...
								{   
									...
									html_data = '<div class="w-full message float-right "><div class="bg-gray-100 chat_message inline-block py-2 lg:py-4 px-5 my-4 rounded-3xl rounded-br-3xl rounded-tr-none float-right"><p class="text-base text-gray-800"> ' + offer_msg + '</p><time class="text-xs text-gray-500 float-right block text-right w-36">' + time + tick + '</time> ' + edit_offer + '</div> </div>';
								}
							}
							if (data.image != "") {
							...
							if (data.msg != "") {
							   ...
							   // denied
									...
									html_data = '<div class="w-full message float-left"><div class="bg-green-500 chat_message inline-block py-2 lg:py-4 px-5 my-4 rounded-3xl rounded-tr-3xl rounded-br-none"><p class="text-base text-white">' + offer_msg + '</p><time class="text-xs text-white float-right block text-right w-36 ">' + time + '</time></div></div>';
								}
							}
							if (data.image != "") {
