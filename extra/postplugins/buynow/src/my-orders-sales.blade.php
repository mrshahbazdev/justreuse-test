	...
	<div class="w-full float-left" {{$class_dir}}>
		<div class="w-full float-left">
			<div class="m-auto container px-4">
				<h1 class="text-xl md:text-2xl font-bold text-black my-4 sm:my-6 lg:my-8 uppercase"> {{__('p_my_orders_sales.my orders & sales')}}</h1>
			</div>
		</div>
		
		<div class="w-full float-left  h-48">
			<div class="container mx-auto px-4">
				...
				<ul class="flex flex-wrap mt-6 md:mt-14">
					<li class="p-2 px-0 md:p-0 flex-auto text-center w-1/2 md:w-auto">
						<a href='<?php echo url("/my-buynow/orders") ?>' class="text-sm sm:text-base lg:text-xl  text-black font-bold  <?php echo ($seg == 'orders') ? 'text-black' : 'text-gray-400'; ?> font-bold">{{__('p_my_orders_sales.my orders')}}</a>
						@if($seg == 'orders')
						<p class="border-green-500 border-b-4 mt-2 w-24 m-auto"></p>
						@endif
					</li>
					<li class="p-2 px-0 md:p-0 flex-auto text-center w-1/2 md:w-auto">
						<a href='<?php echo url("/my-buynow/sales") ?>' class="text-sm sm:text-base lg:text-xl  text-black font-bold  <?php echo ($seg == 'sales') ? 'text-black' : 'text-gray-400'; ?> font-bold">{{__('p_my_orders_sales.my sales')}}</a>
						@if($seg == 'sales')
						<p class="border-green-500 border-b-4 mt-2 w-24 m-auto"></p>
						@endif
					</li>
				...
				<div class="w-full bg-white float-left mb-8 sm:mb-12 md:mb-16 lg:mb-20 relative -mt-20 mt-4">
					...
					<div class="bg-white shadow border border-gray-100 mb-4 px-4 sm:px-6 lg:px-12 xl:px-16 py-4 md:py-6 lg:py-9 relative">
						...
						<div class="flex items-center justify-between mb-4">
							...
							<p class="text-black text-xs sm:text-sm md:text-base font-semibold w-1/2">{{__('p_my_banner_ads.status')}} : <span class="uppercase rounded p-1 {{$status_class}}">{{$order->order_status}}</span></p>
							
							<p class="text-black text-xs sm:text-sm md:text-base font-semibold w-1/2 {{$class_dir_text_rl}}">{{__('p_my_orders_sales.order initiated on')}} : {!! date('d M Y', strtotime($order->created_at)) !!}</p>
						</div>
						<?php 	
						...
						<div class="w-full sm:flex items-center ">
							<div class="w-full sm:w-8/12">
								<div class="sm:flex items-center sm:m-4 sm:mt-0 sm:space-x-4 text-center {{$class_dir_sm_text_lr}} sm:space-x-4 md:justify-start {{$class_dir_post_os}}">
									<div class="h-24 w-24 sm:h-20 sm:w-20 lg:h-24 lg:w-24 mx-auto sm:m-0 mb-2">
										<a class="h-full w-full inline-block" href="{{$post_url}}">
									...
									<div>
										...
										?>
										<p class="text-base font-semibold text-gray-500 mb-2 capitalize">{{__('p_my_orders_sales.seller')}} : <a class="text-green-500 font-semibold" href="<?php echo $profileUrl; ?>" target="_blank">{{$user_info->name}}</a>
											@if($user_deleted == 1)
										...
										<a href="{{$post_url}}">
											<h3 class="text-xl font-semibold hover:text-green-500">{{$post_info->title}}
												<?php
										...
										@if(!empty($post_info->locality))
										<p class="text-sm md:text-base font-semibold text-gray-500 mt-2"> {{$post_info->locality}}</p>
										@endif
									</div>
								</div>
							</div>
							<div class="w-full sm:w-4/12 text-center {{$class_dir_text_rl}} sm:m-auto">
								<div class="sm:m-4 sm:mt-0 mb-4 mt-4">
									<h3 class="text-lg text-gray-900 mb-2 md:mb-4">{{__('p_my_orders_sales.order id')}} : <b>{{$order->id}}</b></h3>
									<a href="<?php echo URL::to('/vieworder/' . $order->orderId); ?>" class="bg-green-500 text-white text-base font-medium uppercase px-4 py-2 rounded shadow outline-none focus:outline-none ease-linear transition-all duration-500 inline-block hover:bg-white hover:text-green-500 border-2 border-green-500">{{__('p_myads.view')}}</a>
								</div>
							</div>
						</div>
						...
						<div class="bg-green-50 py-2 px-4">
							<div class="flex flex-wrap space-x-2 justify-end {{$class_dir_space_r}}">
								@if($order->order_status == "pending")
								<button class="update_order_status text-white bg-red-500 border border-red-500 text-sm font-bold uppercase rounded shadow outline-none hover:bg-white hover:text-red-500 focus:outline-none ease-linear transition-all duration-500 p-2" data-id="{{$order->id}}" data-status="cancelled">{{__('post_detail.cancel')}}</button>
								@endif
								<?php 
				...
				@endif
					...
					<div class="bg-white shadow border border-gray-100 mb-4 px-4 sm:px-6 lg:px-12 xl:px-16 py-4 md:py-6 lg:py-9 relative">
						...
						<div class="flex items-center justify-between mb-4">
							...
							?>
							<p class="text-black text-xs sm:text-sm md:text-base font-semibold w-1/2">{{__('p_my_banner_ads.status')}} : <span class="uppercase rounded p-1 {{$status_class}}">{{$sale->order_status}}</span></p>
							
							<p class="text-black text-xs sm:text-sm md:text-base font-semibold w-1/2 {{$class_dir_text_rl}}">{{__('p_my_orders_sales.order initiated on')}} : {!! date('d M Y', strtotime($sale->created_at)) !!}</p>
						</div>
						<?php 	
						if(!empty($is_deleted_post) || $check_post_package['expired'] == "Expired")
							{
						...
						<div class="w-full sm:flex items-center">
							<div class="w-full sm:w-8/12">
								<div class="sm:flex items-center sm:m-4 sm:mt-0 sm:space-x-4 text-center {{$class_dir_sm_text_lr}} sm:space-x-4 md:justify-start {{$class_dir_post_os}}">
									...
									<div>
										<a href="{{$post_url}}">
											<h3 class="text-xl font-semibold hover:text-green-500 mb-2">{{$post_info->title}}
												<?php
										...
										<p class="text-base font-semibold text-gray-500 mb-2 capitalize">{{__('p_profile.name')}} : {{$user_info->name}}
										@if($user_deleted == 1)
											<span class="bg-yellow-500 text-white rounded text-xs p-1 inline-block">User Deleted</span>
											@endif
											@if($user_blocked == 1)
										...
										<?php 
											...
											?>
										<p class="text-base font-semibold text-gray-500 mb-2">{{__('p_profile.email')}} : {{$hide_email}}</p>
									</div>
								</div>
							</div>
							<div class="w-full sm:w-4/12 text-center {{$class_dir_text_rl}} sm:m-auto">
								<div class="sm:m-4 sm:mt-0 mb-4 mt-4">
									<h3 class="text-lg text-gray-900 mb-2 md:mb-4">{{__('p_my_orders_sales.order id')}} : <b>{{$sale->id}}</b></h3>
									<a href="<?php echo URL::to('/vieworder/' . $sale->orderId); ?>" class="bg-green-500 text-white text-base font-medium uppercase px-4 py-2 rounded shadow outline-none focus:outline-none ease-linear transition-all duration-500 inline-block hover:bg-white hover:text-green-500 border-2 border-green-500">{{__('p_myads.view')}}</a>
								</div>
							</div>
						</div>
						@if($sale->order_status == "pending" || $sale->order_status == "processing" || $sale->order_status == "shipped")
						<div class="bg-green-50 py-2 px-4">
							<div class="flex flex-wrap space-x-2 justify-end {{$class_dir_space_r}}">
								@if($sale->order_status == "pending")
								<button class="update_order_status text-black bg-yellow-500 text-base font-medium uppercase rounded shadow inline-block outline-none focus:outline-none ease-linear transition-all duration-500 px-4 py-2 pb-3 hover:bg-white hover:text-yellow-500 border-2 border-yellow-500" data-id="{{$sale->id}}" data-status="processing">{{__('p_my_orders_sales.mark as processing')}}</button>
								@endif
								@if($sale->order_status == "processing")
								<button class="text-white bg-pink-500 text-base font-medium uppercase rounded shadow outline-none focus:outline-none ease-linear transition-all duration-500 px-4 py-2 hover:bg-white hover:text-pink-500 border-2 border-pink-500 view_courier_info" data-popup-id="courier_info_popup" data-id="{{$sale->id}}">{{__('p_my_orders_sales.mark as shipped')}}</button>
								@endif
								@if($sale->order_status == "shipped")
								<button class="text-white bg-green-500 text-base font-medium uppercase rounded shadow outline-none focus:outline-none ease-linear transition-all duration-500 px-4 py-2 pb-3 border-2 border-green-500  hover:bg-white hover:text-green-500 view_courier_info" data-popup-id="{{$sale->id}}_courier_info_popup" data-id="{{$sale->id}}">{{__('p_my_orders_sales.edit tracking details')}}</button>
								@endif
							</div>
						</div>
						@endif
					...
					<div class="fixed z-50 inset-0 overflow-y-auto" id="{{$sale->id}}_courier_info_popup" style="display:none">
						<div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0" {{$class_dir}}>
							...
							<div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
								<div class="bg-white px-6 py-4 sm:px-12 sm:py-8">
									<div class="{{$class_dir_sm_text_lr}}">
										...
										<div class="w-full inline-block mt-2">
											<div class="form-group mb-4">
												<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.shipment date')}} : <span class="text-red-500">*</span></label>
											...
											<div class="form-group mb-4">
												<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.shipment method')}} : <span class="text-red-500">*</span></label>
												<input type="text" placeholder="{{__('p_my_orders_sales.enter the courier')}}" value="{{$get_courier->courier_name}}" class="s_method text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1" />
											</div>
											<div class="form-group mb-4">
												<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.shipment service')}} : <span class="text-red-500">*</span></label>
												<input type="text" placeholder="{{__('p_my_orders_sales.shipping services')}}" value="{{$get_courier->courier_service}}" class="s_service text-sm md:text-base appearance-none border-l-2 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1" />
											</div>
											<div class="form-group mb-4">
												<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.tracking id')}} : <span class="text-red-500">*</span></label>
												<input type="text" placeholder="{{__('p_my_orders_sales.enter tracking id')}}" value="{{$get_courier->tracking_id}}" class="s_track_id text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1" />
											</div>
											<div class="form-group mb-4">
												<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.additional notes')}} : </label>
												<textarea rows="3" class="s_add_notes text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1">{{$get_courier->more_info}}</textarea>	
											</div>
										...
										<div class="{{$class_dir_sm_text_lr}}">
											...
											<div class="w-full inline-block mt-2">
												...
												<div class="form-group mt-4">
													<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.shipment method')}} : <span class="text-red-500">*</span></label>
													<input type="text" placeholder="{{__('p_my_orders_sales.enter the courier')}}" required class="s_method text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1" />
												</div>
												<div class="form-group mt-4">
													<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.shipment service')}} : <span class="text-red-500">*</span></label>
													<input type="text" placeholder="{{__('p_my_orders_sales.shipping services')}}" required class="s_service text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1"/>
												</div>
												<div class="form-group mt-4">
													<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.tracking id')}} : <span class="text-red-500">*</span></label>
													<input type="text" placeholder="{{__('p_my_orders_sales.enter tracking id')}}" required class="s_track_id text-sm md:text-base appearance-none border-l-2 border-gray-400 rounded w-full px-2 py-3 bg-gray-100 text-black leading-tight focus:outline-none mt-1" />
												</div>
												<div class="form-group mt-4">
													<label class="block text-sm md:text-base text-gray-500 font-semibold mb-1">{{__('p_my_orders_sales.additional notes')}} : </label>
