...
<footer class="text-black" style="background:#fffbfa !important;">
  <div class="max-w-7xl mx-auto px-6 py-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8" >
      ...
      <div>
        <h4 class="font-semibold mb-4">{{__('messages.popular locations')}}</h4>
        <ul class="space-y-2 text-sm">
          @if(!empty($get_country))
            @foreach($pop_locations as $key=>$pop_location)
              <li><a href="#" class="hover:text-primary transition">{{$key}}</a></li>
            @endforeach
          @else
            @foreach($pop_locations as $pop_location)
              ...
              ?>
              <li><a href="{{$popular_loc}}" class="hover:text-primary transition">{{$pop_location}}</a></li>
            @endforeach
          @endif
        </ul>
      </div>
      ...
      <!-- Trending Searches -->
      <div>
        <h4 class="font-semibold mb-4">{{__('messages.trending locations')}}</h4>
        <ul class="space-y-2 text-sm">
          @if(!empty($get_country))
            @foreach($trending_locations as $trending_location)
              ...
              ?>
              <li><a href="{{$trending_loc}}" class="hover:text-primary transition">{{$trending_location['city_name']}}</a></li>
            @endforeach
          @else
            @foreach($trending_locations as $trending_location)
              <li><a href="#" class="hover:text-primary transition">{{$trending_location}}</a></li>
            @endforeach
          @endif
        </ul>
      </div>
      ...
      <!-- About Us -->
      <div>
        <h4 class="font-semibold mb-4">{{__('messages.about us')}}</h4>
        <ul class="space-y-2 text-sm">
          <li><a href="{{ URL::to('/contact-us') }}" class="hover:text-primary transition">{{__('messages.contact us')}}</a></li>
          <li><a href="{{ URL::to('/pages/help') }}" class="hover:text-primary transition">{{__('messages.help')}}</a></li>
          <li><a href="{{ URL::to('/pages/terms-conditions') }}" class="hover:text-primary transition">{{__('messages.terms & conditions')}}</a></li>
          <li><a href="{{ URL::to('/pages/privacy-policy') }}" class="hover:text-primary transition">{{__('messages.privacy policy')}}</a></li>
        </ul>
      </div>

      <!-- Social + Back to top -->
      <div class="flex flex-col items-start">
        <h4 class="font-semibold mb-4">{{__('messages.follow us')}}</h4>
