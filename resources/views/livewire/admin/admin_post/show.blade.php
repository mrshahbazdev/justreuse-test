            ...
            <table class="table-auto w-full">
                ...
                <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
                    ...
                    <tr>
                        ...
                        <td class="border-b px-4 py-2"><a class="" href="{{$posturl}}" target="_blank">{{$row["title"]}}
                        ...
                        <td class="border-b border-gray-300 p-1 text-center">
                            <img width="60px" height="60px" class="inline-block" src="{{$imgUrlfinal}}">
                        </td>
                        ...
                        <td class="border-b px-4 py-2 text-center text-sm">{{$posted_on}}</td>
                        <?php 
                        
                        $check_post_package = App\Models\TblPost::check_post_expired_admin($row->id);
                            ?>
                        <td class="border-b px-4 py-2 text-center text-sm"> {{$check_post_package['to_date']}}</td>
                        <td class="border-b px-4 py-2 text-center">
                            @if(auth()->user()->can('postlist-edit'))
                            <button wire:click="edit('{{ $row->id }}')" class="items-center px-2 py-1 bg-green-500 border border-transparent rounded-sm text-xs text-white  tracking-widest hover:bg-orange-500 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:shadow-outline-indigo disabled:opacity-25 transition ease-in-out duration-150 my-1"><i class="far fa-edit"></i> Post</button>
                            @endif
                            @if(auth()->user()->can('postlist-edit'))
                            <button wire:click="editUser('{{ $row->user_id }}')" class="items-center px-2 py-1 bg-black border border-transparent rounded-sm text-xs text-white tracking-widest hover:bg-gray-500 active:bg-gray-500 focus:outline-none focus:border-gray-500 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 mx-1 my-1"><i class="far fa-edit"></i> User</a></button>
                            @endif
                            @if(auth()->user()->can('postlist-block'))
                            <button wire:click="block_post('{{ $row->id }}')" onclick="confirm('Are you sure you want to {{$confirmName}} this post?') || event.stopImmediatePropagation()" class="{{$className}} items-center px-2 py-1 border border-transparent rounded-sm text-xs text-white tracking-widest active:bg-red-900 focus:outline-none focus:border-red-900 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 mx-1 my-1">{{$btnName}}</a></button>
                            @endif
                        </td>
                    </tr>
                    @endforeach
