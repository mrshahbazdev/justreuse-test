        ...
        <form wire:submit.prevent="update(Object.fromEntries(new FormData($event.target)))">
            <div class="bg-gray-50 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 text-left border rounded shadow mt-4">
                <div class="flex flex-col mb-4">
                    <h1 class="text-2xl font-medium text-gray-900">Update Advertising info</h1>
                </div>
                <div class="flex flex-col mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">
                    Tracking code *</label>
                    <textarea required="required" name="tracking_code" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="10" placeholder="Enter the code here. You need include &lt;script&gt; ... &lt;/script&gt; tags"><?php echo $editdata['tracking_code'];?></textarea>
                </div>             
                <input type="hidden" name="ad_id" value="<?php echo $editdata['id'];?>"/>    
            </div>
