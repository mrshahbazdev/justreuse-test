  ...
  <div class="px-4 md:px-10 mx-auto w-full">
    ...
    <div class="bg-white p-5 rounded-md px-0 overflow-x-auto">
      ...
      <div class="text-sm font-bold mx-5 items-center bg-gray-100 flex justify-between md:flex-no-wrap flex-wrap pl-2 pr-2 mb-2 rounded">
        @if(auth()->user()->can('contact-us-delete-all'))
        ...
        <div class="absolute z-50 pl-3"><i class="fas fa-search text-gray-400 "></i> </div>
          <input type="text" wire:model="search" class="px-3 py-3 placeholder-gray-400 text-gray-700 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline w-full pl-10" placeholder="Search Here">
        </div>
      </div>
        ...
        <table class="table-auto w-full text-sm">
          <thead>
            <tr class="border border-l-0 border-r-0">
              ...
              <th class="border-b px-4 py-2 text-transform: uppercase text-xs">Email</th>
              <th class="border-b px-4 py-2 text-transform: uppercase text-xs">Attachment</th>
              <th class="border-b px-4 py-2 text-transform: uppercase text-xs">Phone</th>
          ...
          <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
            ...
            <tr>
              ...
              <td class="border-b px-4 py-2 text-center">{{$row->email}}
                @if($row->ad_link != '')
                <p class="text-sm">Ad link: <a class="underline" href="{{$row->ad_link}}" target="_blank">{{Str::limit($row->ad_link, 20)}}</a></p>
                @elseif($row->title != '')
                <p class="text-sm">Title: {{Str::limit($row->title, 20)}}</p>
                @endif
              </td>
              <td class="border-b px-4 py-2 text-center">
                <?php
                $imageUrl = URL::to('storage/' . $row->attachment);
                $no_img = URL::to('storage/noimage150.png');
                ?>
                @if($row->attachment == '')
                <img src="{{$no_img}}" width="70" height="70" alt="attachment" />
                @else
                <img src="{{$imageUrl}}" width="70" height="70" alt="attachment" />
                @endif

              </td>
              <td class="border-b px-4 py-2 text-center">{{$row->phone}}</td>
              <td class="border-b px-4 py-2 text-center">{{ Str::limit($row->description, 20) }}</td>
              <td class="border-b px-4 py-2 text-center">
                @if(auth()->user()->can('contact-us-view'))
                <button class="items-center px-2 py-1 border border-transparent rounded-md text-xs text-white  tracking-widest  focus:outline-none focus:shadow-outline-orange disabled:opacity-25 transition ease-in-out duration-150 <?php echo $fav_style; ?>" data-report-id="{{$row['id']}}" id="view_report"><i class="far fa-eye"></i></button>
                @endif
                @if(auth()->user()->can('contact-us-delete-single'))
                <button wire:click="deleteReq('{{ $row->id }}')" class="items-center px-2 py-1 bg-red-700 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-red-500 active:bg-red-900 focus:outline-none focus:border-red-900 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 ml-5"><i class="far fa-trash-alt"></i></a></button>
                @endif
              </td>
            </tr>
            @endforeach
          ...
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-headline"> Details </h3>
                <div class="mt-2">
