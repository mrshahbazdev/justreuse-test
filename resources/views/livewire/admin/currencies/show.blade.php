...
<div class="relative md:pt-28 pb-32 pt-12">
    <div class="px-4 md:px-10 mx-auto w-full">
        <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg p-5 px-0">
            ...
            <!-- end search -->
            @if($insertMode)
            @include('livewire.admin.currencies.create')
            @endif
            ...
            <table class="w-full">
                ...
                <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
                    ...
                    <tr>
                        <td class="border-b px-4 py-2">{{ $currency->currency_name }}</td>
                        <td class="border-b px-4 py-2 whitespace-normal">{{ $currency->short_code }}</td>
                        <td class="border-b px-4 py-2 text-center"><?php echo $currency->currency_hex; ?></td>
                        <td class="border-b px-4 py-2 text-center">
                            @if(auth()->user()->can('currency-delete'))
                            <button wire:click="deleteReq('{{ $currency->id }}')" class="bg-red-500 hover:bg-red-700 text-white px-2 py-1 text-xs rounded-sm delete focus:outline-none focus:border-red-900 focus:shadow-outline-red transition ease-in-out duration-150"><i class="far fa-trash-alt"></i></button>
                            @endif
                        </td>
                    </tr>
                    @endforeach
