<div class="relative md:pt-28 pb-32 pt-12">
	...
	<div class="px-4 md:px-10 mx-auto w-full">
			...
			<form wire:submit.prevent="update(Object.fromEntries(new FormData($event.target)))">
				...
				<div>
					...
					<div class="inline-block w-2/4">
						<input type="text" name="cattitle" readonly="readonly" id="id_cattitle" value="{{ $data['cat_title'] }}" class="form-input rounded-md shadow-sm block mt-1 w-full">
						<input type="hidden" name="catid" id="id_catid" value="{{ $data['cat_id'] }}" class="form-input rounded-md shadow-sm block mt-1 w-full">
						<input type="hidden" name="cathtml" id="id_cathtml" value="{{ $data['html'] }}" />
						<input type="hidden" name="catfieldcount" id="id_catfieldcount" value="{{ $data['field_count'] }}" />
					</div>
		...
		@include('livewire.admin.customfields.js_css')
	</div>
</div>