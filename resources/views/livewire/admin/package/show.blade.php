            ...
            <table class="table-auto w-full text-md">
                <thead>
                    <tr class="border border-l-0 border-r-0">
                        ...
                        @if(auth()->user()->can('package-edit') || auth()->user()->can('package-delete'))
                ...
                <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
                    ...
                    <tr align="center">
                        <td class="border-b px-4 py-2 text-sm text-left">{{$row->name}}
                            <span class="text-xs font-semibold inline-block py-0.5 px-2 rounded text-pink-600 bg-pink-200 ml-2 mr-2">{{str_replace('_', ' ', $row->ad_type)}}</span>
                        </td>
                        <td class="border-b px-4 py-2">{{$row->price}}</td>
                        <td class="border-b px-4 py-2">
                            @if($row->bulk_ads == 1)
                            <p class="text-sm">{{$row->bulk_limit}} Ads</p>
                            <?php
                            $bulk_type_title = "";
                            if ($row->bulk_type == "1") {
                            ...
                            ?>
                            <p class="text-sm">{{$bulk_type_title}}</p>
                            @endif
                        </td>
                        <td class="border-b px-4 py-2">{{$row->duration}} days</td>
                        ...
                        <td class="border-b px-4 py-2">
                            ...
                            <?php if ($row->lft != 1) { ?>
                                <button wire:click="deleteReq('{{ $row->id }}')" class=" bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded delete text-xs focus:outline-none focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150"><i class="far fa-trash-alt"></i></button>
                            <?php } ?>
                            @endif
                        </td>
                    </tr>
