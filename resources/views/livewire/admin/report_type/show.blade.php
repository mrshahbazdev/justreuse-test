    ...
    <div class="px-4 md:px-10 mx-auto w-full">
        ...
        <div class="bg-white p-5 rounded-md px-0 overflow-x-auto">
            ...
            <div class="text-sm font-bold mx-5 items-center bg-gray-100 flex justify-between md:flex-no-wrap flex-wrap pl-2 pr-2 mb-2 rounded">
                @if(auth()->user()->can('reporttype-create'))
            ...
            <table class="table-auto w-full">
                ...
                <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
                    ...
                    <tr>
                        ...
                        <td class="border-b px-4 py-2 text-center">{{$row->type}}</td>
                        <td class="border-b px-4 py-2 text-center">{{ $row->created_at->format('d-m-Y h:i a') }}</td>
                        <td class="border-b px-4 py-2 text-center">
                            @if(auth()->user()->can('reporttype-edit'))
                            <button wire:click="edit('{{ $row->id }}')" class="items-center px-2 py-1 bg-green-500 border border-transparent rounded-sm text-xs text-white  tracking-widest hover:bg-orange-500 active:bg-orange-500 focus:outline-none focus:border-orange-500 focus:shadow-outline-orange disabled:opacity-25 transition ease-in-out duration-150"><i class="far fa-edit"></i></button>
                            @endif
                            @if(auth()->user()->can('reporttype-delete'))
