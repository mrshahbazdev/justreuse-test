  ...
  <div class="px-4 md:px-10 mx-auto w-full">
    ...
    <div class="bg-white p-5 rounded-md px-0 overflow-x-auto">
      ...
      <div class="text-sm font-bold mx-5 items-center bg-gray-100 flex justify-between md:flex-no-wrap flex-wrap pl-2 pr-2 mb-2 rounded">
        @if(auth()->user()->can('reportuser-delete'))
      ...
      <table class="table-auto w-full">
        <thead>
          <tr class="border border-l-0 border-r-0">
            ...
            @if(auth()->user()->can('reportuser-view'))
        ...
        <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
          ...
          <tr>
            ...
            <td class="border-b px-4 py-2 text-center">{{$row->user_name}}</td>
            <td class="border-b px-4 py-2 text-center">{{$row->reported_user_name}}</td>
            <td class="border-b px-4 py-2 text-center">{{$row->report_name}}</td>
...
<script>
  ...
  $(document).on("click", "#view_report", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "{{ URL::to('report-user-comment') }}",
      data: {
        id: id
      },
