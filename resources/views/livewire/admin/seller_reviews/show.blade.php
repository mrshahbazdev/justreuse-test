...
@if(auth()->user()->can('seller_review-list'))
<div class="relative md:pt-28 pb-32 pt-12">
  <div class="px-4 md:px-10 mx-auto w-full">
    ...
    <div class="bg-white p-5 rounded-md px-0">
      ...
      @if(auth()->user()->can('seller_review-delete'))
        <button class="bg-green-500 hover:bg-orange-500 text-white font-bold py-2 px-4 rounded my-3 focus:outline-none focus:shadow-outline-orange disabled:opacity-25 transition ease-in-out duration-150"><i class="far fa-trash-alt"></i></button>
        @endif
        <!-- search -->
      ...
      <table class="table-auto w-full text-sm">
        ...
        <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
          <?php $i = 0; ?>
          @foreach($seller_review_list as $row)
          <?php
          ...
          <tr>
            ...
            <p><strong>Seller Name:</strong> {{$row->seller_name}} </p>
            ...
            <td class="border-b px-4 py-2 text-center">
              @if(auth()->user()->can('seller_review-approve'))
              <a href="{{route('seller_review_approve',$row->id)}}"><button class="view_review items-center px-2 py-1 border border-transparent rounded-md text-xs text-white  tracking-widest  focus:outline-none focus:shadow-outline-orange disabled:opacity-25 transition ease-in-out duration-150 <?php echo $fav_style; ?>" data-val="{{$row->approved}}">{{$is_approved}}</button></a>
              @endif
            </td>
            <td class="border-b px-4 py-2 text-center">{{ $row->created_at->format('d-m-Y h:i a') }}</td>
            <td class="border-b px-4 py-2 text-center flex justify-center">
              @if(auth()->user()->can('seller_review-view'))
              <button class="view_report items-center px-2 py-1 border border-transparent rounded-md text-xs text-white  tracking-widest  focus:outline-none focus:shadow-outline-orange  bg-orange-500 disabled:opacity-25 transition ease-in-out duration-150 <?php echo $review_style; ?>" data-report-id="{{$row['id']}}"><i class="far fa-eye"></i></button>
              @endif

              @if(auth()->user()->can('seller_review-delete-single'))
              <form action="{{ route('seller_review_delete',$row->id) }}" method="POST">
                    @csrf
              <button type="submit" class="items-center px-2 py-1 bg-red-500 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 ml-5" onclick="return confirm('Are you sure you want to delete this item?');"><i class="far fa-trash-alt"></i></button>
                   </form>
              @endif
            </td>
          </tr>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  ...
                  <div>
                    ...
                    <p id="fill_comment">{{$row->comment}}</p>
                  </div>
...
<script>
  ...
  //     type: 'POST',
  //     dataType: 'json',
  //     url: "{{ URL::to('seller-approve-review') }}",
  //     data: {
  //       id: id,
  //       value: approve_val
  //     },
