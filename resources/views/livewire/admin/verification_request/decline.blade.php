...
<div class="fixed z-10 inset-0 overflow-y-auto ease-out duration-400" style="z-index:100; position: fixed;">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <form method="post" action="{{route('admin/verification_request/decline-approve',$id)}}" id="lang_submit">
                @csrf
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mb-4">
                        <h1 class="text-2xl font-bold">Reason for Decline
                        </h1>
                        <textarea id="w3review" name="reason" rows="4" cols="45"></textarea>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                        <a href="{{ route('admin/verification-request') }}"><button type="button" class="inline-flex justify-center w-full rounded-md border border-gray-300 px-4 py-2 bg-white text-base leading-6 font-medium text-gray-700 shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                            Cancel
                        </button></a>
                    </span>
                    </div>
