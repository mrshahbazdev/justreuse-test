	...
	?>
		...
		<div class="w-full float-left">
			<div class="container mx-auto px-4">
				<h1 class="text-xl md:text-2xl font-bold text-black my-4 sm:my-6 lg:my-8 uppercase">{{__('p_choose_package.single package')}}</h1>
			</div>
		</div>
		...
		<div class="w-full float-left">
			<div class="container mx-auto px-4">
				<div class="bg-green-50 shadow-md rounded relative mx-auto lg:max-w-4xl w-full mb-8 sm:mb-12 md:mb-16 lg:mb-20 md:p-8 lg:p-12 p-4 ">
					<div class="w-full float-left sm:float-none sm:flex sm:items-center sm:flex-wrap sm:space-x-8 {{$class_dir_sm_space_r}}">
						<div class="sm:float-left md:float-none sm:mb-0 mb-4">
							<div class="flex items-center justify-center mx-auto h-36 w-36">
								<img class="w-full h-36 rounded-lg mx-auto object-cover object-center" src="{{ $postimg }}" />
							</div>
						</div>
						<div class="{{$class_dir_sm_text_lr}} text-center">
							<h1 class="text-lg sm:text-xl lg:text-2xl text-black mb-2 sm:mb-3">{{$post_info[0]->title}}</h1>
							<p class="text-lg lg:text-xl text-black font-bold">
								<?php echo !empty($currency_symbol) ? $currency_symbol['currency_hex'] : ""; ?> <span id="">{{$post_info[0]['price']}}</span>
							</p>
						</div>
					</div>
					
					<div class="inline-block md:block mt-5 md:mt-8 lg:mt-10 sm:mb-0 mb-4">
						<h2 class="text-gray-500 font-bold text-lg md:text-2xl mb-6 lg:mb-8 mx-auto">{{__('p_choose_package.choose package')}}</h2>
						<div class="mt-2">
							<?php
							$i = 0;
							...
							?>
								<div class="inline-block align-middle">
									<label class="{{$class_dir_mar_lr}} inline-flex items-center mb-3 sm:mb-6">
										<input type="radio" class="form-radio text-indigo-600 w-4 h-4 md:w-6 md:h-6" name="radio_pack" data-pack-id="{{$k['id']}}" data-pack-value="{{$k['price']}}" data-pack-days="{{$k['duration']}}" value="{{$k['id']}}" {{$checkString}}>
										<span class="mx-2 text-base text-black font-medium">{{$k['name']}} ( {{$k['duration']}} {{__('p_choose_package.days')}} )</span>
									</label>
								</div>
							<?php
								$i++;
						...
						<input type="hidden" id="selected_pack_id" value="{{$list_of_packs[0]['id']}}" />
						<input type="hidden" id="selected_pack_amount" value="{{$list_of_packs[0]['price']}}">
						<input type="hidden" id="selected_pack_days" value="{{$list_of_packs[0]['duration']}}">
					</div>
					
					<div class="block md:mt-4 lg:mt-8">
						<h2 class="text-gray-500 font-bold text-lg md:text-2xl mb-3 sm:mb-6 lg:mb-8">{{__('p_choose_package.payment type')}}</h2>
						<div class="mt-3 md:flex 	">
							...
							?>
								<div class="inline-block align-middle">
									<label class="{{$class_dir_mar_lr}} inline-flex items-center mb-2">
										<input type="radio" class="form-radio text-indigo-600 w-4 h-4 md:w-6 md:h-6" name="payment_type" value="{{$p['name']}}" {{$checkString}}>
										@if(strtolower($p['display_name']) == "paypal")
										<span><img src="{{ URL::to('/images/web_paypal.png') }}" class="h-10 md:h-12 mx-2" /></span>
										@elseif(strtolower($p['display_name']) == "stripe")
										<span><img src="{{ URL::to('/images/stripe_logo.png') }}" class="h-10 md:h-12 mx-2" /></span>
										@else
					...
					<div class="sm:flex sm:flex-wrap items-center sm:justify-between py-3">
						<h1 class="text-base font-medium capitalize">{{__('p_choose_package.more discounts for business purpose')}}</h1>
						<div class="mt-2 md:mt-0">
							<button class="w-full sm:w-44 bg-green-500 text-base text-white font-medium rounded md:rounded-lg outline-none focus:outline-none shadow hover:shadow-md outline-none hover:bg-white ease-linear transition-all duration-500 border-2 border-green-500 hover:text-green-500"><a class="block pt-2 pb-3 " href="{{URL::to('/selectPackageMultiple')}}">{{__('p_choose_package.view')}}</a></button>
						</div>
					</div>
					<div class="">
						<div class="md:flex md:flex-wrap text-center {{$class_dir_md_text_lr}} items-center  md:py-3">
							<div class="w-full flex-grow flex-1"><button id="show_coupon" class="text-base font-medium outline-none focus:outline-none  underline pt-2 pb-2">{{__('p_choose_package.do you have coupon code')}}</button></div>
							<div class="w-full flex-grow flex-1 {{$class_dir_md_text_rl}}"><span class='text-right'><input style="display:none" id="coupon_code" class="uppercase text-base font-medium focus:outline-none border-1 focus:border-green-500 border border-green-500 text-center px-3 py-2 rounded-lg sm:w-72 lg:w-96 h-12 w-full" type="text" name="coupon_code" placeholder="{{__('p_choose_package.enter coupon code')}}" /></span></div>
						</div>
						<div class="rounded-lg px-4 py-3 bg-white text-sm mt-3" style="display:none" id="sub_window_fixed">
							<div class="flex flex-wrap items-center mb-3">
								...
								<div class="relative w-full px-4 max-w-full flex-grow flex-1"><p class="text-base text-black font-medium" id="coupon_title">{{__('p_choose_package.discount')}}</p></div>
								<div class="relative w-full px-4 max-w-full flex-grow flex-1 {{$class_dir_text_rl}}"><p class="text-base text-black font-medium">- <?php echo !empty($currency_symbol) ? $currency_symbol['currency_hex'] : ""; ?> <span id="coupon_value">0.00</span></p></div>
							</div>
							<div class="flex flex-wrap items-center mb-3">
								<div class="relative w-full px-4 max-w-full flex-grow flex-1"><p class="text-base text-black font-medium">{{__('p_choose_package.sub total')}}</p></div>
								<div class="relative w-full px-4 max-w-full flex-grow flex-1 {{$class_dir_text_rl}}"><p class="text-base text-black font-medium"><?php echo !empty($currency_symbol) ? $currency_symbol['currency_hex'] : ""; ?> <span id="sub_total">{{$list_of_packs[0]['price']}}</span></p></div>
							</div>
							<div class="flex flex-wrap items-center mb-3">
								<div class="relative w-full px-4 max-w-full flex-grow flex-1"><p class="text-base text-black font-medium" id="tax_title">{{__('p_choose_package.tax')}}</p></div>
								<div class="relative w-full px-4 max-w-full flex-grow flex-1 {{$class_dir_text_rl}}"><p class="text-base text-black font-medium"><?php echo !empty($currency_symbol) ? $currency_symbol['currency_hex'] : ""; ?><span id="tax_amt">0.00</span></p></div>
							</div>
							<div class="flex flex-wrap items-center border-b border-t border-gray-800 py-3">
								<div class="relative w-full px-4 max-w-full flex-grow flex-1"><p class="text-base text-black font-medium">{{__('p_choose_package.total')}}</p></div>
								<div class="relative w-full px-4 max-w-full flex-grow flex-1 {{$class_dir_text_rl}}"><p class="text-base text-black font-medium"><?php echo !empty($currency_symbol) ? $currency_symbol['currency_hex'] : ""; ?> <span id="final_total_amount">{{$list_of_packs[0]['price']}}</span></p></div>
							</div>
						</div>
					</div>
					<div class="pt-3 sm:pt-5 w-full">
						<?php if (!empty($currency_symbol) && !empty($payment_methods)) { ?>
							<button id="pay_proceed" class="rounded w-full p-2 pb-3 md:p-3 md:pb-4 lg:p-4 lg:pb-5 bg-green-500 text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-white shadow outline-none focus:outline-none ease-linear transition-all duration-500 border-2 border-green-500 hover:text-green-500 hover:bg-white">{{__('p_choose_package.pay')}} <span class="text-base sm:text-lg md:text-xl lg:text-2xl font-bold">
									<?php echo $currency_symbol['currency_hex']; ?> <span id="show_price_2">{{$list_of_packs[0]['price']}}</span></span></button>
						<?php } else { ?>
							<button class="cursor-not-allowed not-allowed w-full rounded p-4 bg-green-500 text-2xl font-bold text-white shadow outline-none focus:outline-none ease-linear transition-all duration-500 border-2 border-green-500 hover:text-green-500 hover:bg-white">{{__('p_choose_package.pay')}}
							</button>
						<?php } ?>
					</div>
				</div>
