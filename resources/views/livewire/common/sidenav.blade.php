...
<nav class="bg-side-nav md:left-0 md:block md:fixed md:top-0 md:bottom-0 md:overflow-y-auto md:flex-row md:flex-no-wrap md:overflow-hidden shadow-xl bg-white flex flex-wrap items-center justify-between relative md:w-64 z-10 py-4  px-4 lg:pt-0 lg:px-0">
  <div class="md:flex-col md:items-stretch md:min-h-full md:flex-no-wrap px-0 flex items-center justify-between w-full mx-auto">
    ...
    <a class="lg:pt-4 md:block text-center text-gray-700 mr-0 inline-block whitespace-no-wrap text-sm uppercase font-bold" href="{{ URL::to('/admin/dashboard') }}">
      <?php
      // echo !empty($settings) ? $settings['name'] : "Admin Panel";
      ?>
      <img class="max-w-full" src="{{URL::to('storage/'.$settings['admin_logo'])}}">
    </a>
    ...
    <a class="lg:text-gray-800 lg:hover:text-yellow-300 text-gray-800 pl-3 py-4 lg:py-2 flex items-center text-sm xl:hidden" href="#pablo" onclick="openDropdown(event,'demo-pages-dropdown1')">
          <img src="{{$userImgUrl}}" class="bg-white rounded-full mr-1" width="30" height="30" alt="your profile"/></a>
          ...
          <a href="{{ URL::to('admin/my-profile') }}" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">My Profile</a>
          <form method="POST" action="{{ route('logout') }}">
          @csrf
          <a href="{{ route('logout') }}" class="lg:text-gray-800 lg:hover:text-yellow-300 text-gray-800 px-3 py-4 lg:py-2 flex items-center text-sm" onclick="event.preventDefault(); this.closest('form').submit();"><i class="fa fa-sign-out mr-1" fa-lg=""></i> Logout</a>
          </form>    
        </div>
    ...
    <div class="md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 md:shadow-none shadow absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 hidden" id="example-collapse-sidebar">
      <div class="md:min-w-full md:hidden block pb-0 mb-4 border-b border-solid border-gray-300 fixed inset-x-0" style="background-color:#313a46">
        <div class="w-full text-center flex justify-center align-center">
          <div class="w-full flex justify-center align-center">
            <a class="md:block text-center md:pb-2 text-gray-700 mr-0 inline-block whitespace-no-wrap text-sm uppercase font-bold p-4 px-0" href="{{ URL::to('/admin/dashboard') }}">
              <?php
              // echo !empty($settings) ? $settings['name'] : "Admin Panel";
              ?>
              <img class="max-w-full" src="{{URL::to('storage/'.$settings['admin_logo'])}}">
      ...
      <ul class="md:flex-col md:min-w-full flex flex-col list-none mt-32 xl:mt-0">
        ...
        <div x-data="{ open:{{$selected_ads}} }" class="border-b border-gray-600">
          ...
          <div x-show="open" class="bg-gray-500 pb-2 transition" >
            ...
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_posts == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white " href="{{ route('admin/post') }}">Post lists</a>
            @endif
            @if(auth()->user()->can('category-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_category == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/category') }}">Category</a>
            @endif
            @if(auth()->user()->can('category-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm  <?php echo ($open_features == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('features') }}">Features</a>
            @endif
            @if(auth()->user()->can('category-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm  <?php echo ($open_features_map == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('features-map-show') }}">Features Mapping</a>
            @endif
            @if(auth()->user()->can('reportad-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_report_ad == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/report-ad') }}">Report Ads</a>
            @endif
            @if(auth()->user()->can('blocked-post-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_blocked_post == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/blocked-post') }}">Blocked Posts</a>
            @endif
          </div>
        </div>
        ...
        <div x-data="{ open: {{$selected_users}} }" class="border-b border-gray-600">
          ...
          <div x-show="open" class="bg-gray-500 pb-2">
            ...
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_user == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/user') }}">Users</a>
            @endif
            @if(auth()->user()->can('role-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_role == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/role') }}">Roles</a>
            @endif
            @if(auth()->user()->can('permission-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_permission == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/permissions') }}">Permissions</a>
            @endif
            @if(auth()->user()->can('verificationrequest'))
          <a class="py-3 px-6 pl-12 block transition text-sm hidden <?php echo ($open_bulk_email == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/verification-request') }}">Verification Request</a>
          @endif
          </div>
        </div>
        ...
        ?>
        <div x-data="{ open: {{$selected_setup}} }" class="border-b border-gray-600">
          ...
          <div x-show="open" class="bg-gray-500">
            ...
            <a class="py-3 px-6 pl-12 block transition  text-sm  hover:text-white <?php echo ($open_settings == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white " href="{{ route('admin/settings') }}">Settings</a>
            @endif

            @if(auth()->user()->can('post-method-list'))
            <a class="py-3 px-6 pl-12 block transition  text-sm  hover:text-white <?php echo ($open_post_methods == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/post-methods') }}">Post Methods</a>
            @endif

            @if(auth()->user()->can('currency-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_currency_mgnt == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/currency') }}">Currency</a>
            @endif

            @if(auth()->user()->can('package-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_packages == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/package') }}">Package</a>
            @endif
            @if(auth()->user()->can('reporttype-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_report_type == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/report-type') }}">Report Types</a>
            @endif
            @if(auth()->user()->can('payment-methods-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_payment_methods == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/payment-methods') }}">Payment Methods</a>
            @endif
            @if(auth()->user()->can('chat-method-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_chat_methods == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/chat-methods') }}">Chat Methods</a>
            @endif

            @if(auth()->user()->can('coupon-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_coupon == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/coupon') }}">Coupons</a>
            @endif

            @if(auth()->user()->can('language-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_language == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/language') }}">Languages</a>
            @endif

            @if(auth()->user()->can('country-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm  <?php echo ($open_country == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/country') }}">Country</a>
            @endif

            @if(auth()->user()->can('advertising-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_advertising == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/advertising') }}">Advertising</a>
            @endif

            @if(auth()->user()->can('blacklist-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_blacklist == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/blacklist') }}">Blacklist</a>
            @endif

            @if(auth()->user()->can('home-banner-list'))
            <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_home_banner == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white" href="{{ route('admin/home-banner') }}">Home Banner</a>
            @endif

            @if(auth()->user()->can('email-template-list'))
            <a class="py-3 px-6 pl-12 block transition  text-sm <?php echo ($open_email_template == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?>  hover:text-white " href="{{ route('admin/email-template') }}">Email Template</a>
            @endif
        ...
        ?>
        <div x-data="{ open: {{$selected_sales}} }" class="border-b border-gray-600">
          <button @click="open = !open" class="w-full flex justify-between items-center py-3 px-3 text-gray-400 cursor-pointer  hover:text-white focus:outline-none">
            <span class="flex items-center">
          ...
          <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_payments == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/payment') }}">Service Orders</a>
          @endif
          <!-- @if(auth()->user()->can('bulk-payment-list'))
          <a class="py-3 px-6 pl-12 block transition text-sm <//?php echo ($open_bluk_payments == "true") ? "bg-gray-500 text-white" : "text-gray-400 "; ?> hover:text-white" href="{{ route('admin/bulk-payments') }}">Bulk Orders</a>
          @endif -->


            @if(auth()->user()->can('buynow-orders-list'))
          @if(!empty($check_post_methods))
          @if(in_array("buynow", $check_post_methods))
          <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_buynow_orders == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/buynow-orders') }}">Buynow Orders</a>
          @endif
          @endif
          @endif
          ...
          <a class="py-3 px-6 pl-12 block transition text-sm <?php echo ($open_banner_ads == "true") ? "bg-green-500 text-white cursor-pointer  transition hover:text-white focus:outline-none" : "text-gray-400"; ?> hover:text-white" href="{{ route('admin/banner-advertisements') }}">Banner Ads</a>
          @endif
          @endif
          @endif
