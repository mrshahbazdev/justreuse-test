	...
	<div class="relative mt-4 mb-6 sm:mb-8 md:mb-12 lg:mb-16 w-full float-left" {{$class_dir}}>
		<div class="container m-auto px-4">
			<h1 class="text-center font-bold text-2xl lg:text-3xl xl:text-4xl p-3 text-gray-700">{{__('p_packages.packages')}}</h1>
			<p class="text-center text-base text-black mt-1">{{__('p_packages.package quotes')}}.</p>
					...
					<div class="w-full sm:w-6/12 md:w-4/12 lg:w-3/12 md:mb-0 mb-2 px-4 py-2">
						<div class="rounded-tr-lg rounded-tl-lg mt-4 border-2 border-gray-300 shadow-xl">
							...
							<div class="text-center p-4">
								...
								<span class="text-gray font-sm"> / {{__('p_packages.ad')}}</span>
								</h1>
							</div>
							<div class="border-0 border-grey-light border-t border-solid text-sm">
								...
								<p class="text-base font-normal">{{__('p_packages.availability')}} <span class="font-bold">{{ $row->duration }} {{__('p_packages.days')}}</span></p>
								</div>
							</div>
						</div>
					</div>
		...
		<div class="container m-auto px-4">
			<h1 class="text-center font-bold text-2xl lg:text-3xl xl:text-4xl px-3 py-0 lg:py-3 text-gray-700">{{__('p_packages.business package')}}</h1>
			<div class="max-w-6xl mx-auto mb-6 md:mb-10">
				<div class="flex flex-wrap flex-justify mt-2 mb-2">
					@foreach($business_packs as $row)
					<div class="w-full sm:w-6/12 md:w-4/12 lg:w-3/12 md:mb-0 mb-2 px-4 py-2">
						<div class="rounded-tr-lg rounded-tl-lg mt-4 border-2 border-gray-300 shadow-xl">
							<div class="p-3 pb-4 rounded-md text-xl lg:text-2xl text-white font-bold text-center bg-green-500">
							{{ $row->name }}
							...
							<div class="border-0 border-grey-light border-t border-solid text-sm">
								<div class="text-center border-0 border-grey-light border-b border-solid py-4">
								<p class="text-base font-normal">{{__('p_packages.ad limit')}} <span class="font-bold">{{ $row->bulk_limit }}</span></p>
								</div>
								<div class="text-center border-0 border-grey-light border-b border-solid py-4">
								<p class="text-base font-normal">{{__('p_packages.ad type')}} <span class="font-bold"><?php echo str_replace('_', ' ', strtoupper($row->ad_type)); ?></span></p>
								</div>
								<div class="text-center border-0 border-grey-light border-b border-solid py-4">
								<p class="text-base font-normal">{{__('p_packages.availability')}} <span class="font-bold">{{ $row->duration }} {{__('p_packages.days')}}</span></p>
								</div>
		...
		<div class="container m-auto px-4">
			...
			<h1 class="text-center font-bold text-4xl p-3 mt-6 pb-0">{{__('p_packages.coupons')}}</h1>
					...
					<div class="w-full sm:w-6/12 md:w-4/12 lg:w-3/12 md:mb-0 mb-2 px-4 py-2">
						<div class="rounded-tr-lg rounded-tl-lg mt-4 border-2 border-gray-300 shadow-xl">
							<div class="p-3 rounded-t-md rounded-l-md text-2xl border-0 font-bold text-center bg-yellow-500">
								<p class="text-base text-black">{{ $coupon->coupon_title }}</p>
							</div>
							<div class="text-center p-4">
								<h1 class="text-xl text-black font-bold">
								<span class="text-sm font-normal">{{__('p_packages.coupon code')}} :</span> {{ $coupon->coupon_code }}
								</h1>
							</div>
							<div class="border-0 border-grey-light border-t border-solid text-sm">
								<div class="text-center border-0 border-grey-light border-b border-solid py-4">
									<p class="text-base font-normal">{{__('p_packages.offer')}} :
								...
								<div class="text-center border-0 border-grey-light border-b border-solid py-4">
									<p class="text-base font-normal">{{__('p_packages.validity')}} : <span class="font-bold"><?php echo date("d M Y", strtotime($coupon->start_date)); ?> - <?php echo date("d M Y", strtotime($coupon->end_date)); ?></span></p>
								</div>
							</div>
						</div>
					</div>
