        ...
        <aside class="sidebar">
            ...
            <div class="filter-section active">
                ...
                @foreach($groupedFilters as $groupName => $filtersInGroup)
                    <div class="main-filter-group" x-data="{ open: false }" wire:key="main-group-{{ $groupName }}">
                        <h2 @click="open = !open" class="main-group-heading">
                            <span><i class="{{ $this->getGroupIcon($groupName) }}"></i> {{ $groupName }}</span>
                        ...
                        <div x-show="open" x-collapse.duration.300ms>
                            @foreach($filtersInGroup as $filter)
                                ...
                                @endphp
                                <div class="filter-group" wire:key="filter-group-{{ $filter->id }}">
                                    <h3 wire:click="openFilterModal({{ $filterIndex }})" class="filter-heading">
                                    ...
                                    <div>
                                        @if($filter->id === 'main_categories' && $categorySlug)
                                            <div class="tags-container">
                                                <span class="tag-item">
                                                    {{ $allCategories->firstWhere('slug', $categorySlug)->title ?? '' }}
                                        ...
                                        @elseif(!in_array($filter->type, ['price', 'distance', 'radio', 'main_category_radio']) && !empty($customFilters[$filter->id]) && is_array($customFilters[$filter->id]) && count($customFilters[$filter->id]) > 0)
                                            <div class="tags-container">
                                                @foreach($customFilters[$filter->id] as $selectedOption)
                                                    <span class="tag-item">
                                                        {{ $selectedOption }}
                                                        <button wire:click="removeCustomFilter('{{ $filter->id }}', '{{ $selectedOption }}')" title="Remove">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h2 class="modal-title">
                    ...
                    @if($this->currentFilter->type === 'distance')
                        ({{ $distance }} km)
                    @endif
                </h2>
            ...
            <div class="modal-body">
                @if($this->currentFilter->type === 'price')
                    <div class="price-filter" style="padding: 20px 10px;">
                        ...
                        <div class="price-values"><span> {{ number_format($minPrice) }}</span><span> {{ number_format($maxPrice) }}</span></div>
                    </div>
                @elseif($this->currentFilter->type === 'distance')
                     <div class="distance-filter" style="padding: 20px 10px;">
                        ...
                        <div class="distance-values"><span>500 km</span><span>{{ $maxDistance }} km</span></div>
                    </div>
                @else
                    ...
                    <ul class="modal-options-list">
                        @if($this->currentFilter->type === 'main_category_radio')
                            @foreach($this->currentFilter->options->filter(function($option) {
                                return empty($this->modalSearchTerm) || stripos($option->title, $this->modalSearchTerm) !== false;
                            }) as $option)
                                <li wire:key="modal-option-main-cat-{{ $option->id }}">
                                    <label class="modal-option">
                                        <input type="radio" wire:model.live="categorySlug" name="categorySlugModal" value="{{ $option->slug }}">
                                        <span class="option-label">{{ $option->title }}</span>
                                        <span class="custom-radio"></span>
                                    </label>
                                </li>
                            @endforeach
                        @elseif($this->currentFilter->type === 'radio')
                             @foreach($this->currentFilter->options->filter(function($option) {
                                return empty($this->modalSearchTerm) || stripos($option->title, $this->modalSearchTerm) !== false;
                            }) as $option)
                                <li wire:key="modal-option-sub-cat-{{ $option->id }}">
                                    <label class="modal-option">
                                        <input type="radio" wire:model.live="selectedSubCategory" name="selectedSubCategoryModal" value="{{ $option->slug }}">
                                        <span class="option-label">{{ $option->title }}</span>
                                        <span class="custom-radio"></span>
                                    </label>
                                </li>
                        ...
                        @else
                            @foreach($this->currentFilter->options->filter(function($option) {
                                ...
                                <li wire:key="modal-option-{{ $option->id }}">
                                    <label class="modal-option">
                                        <input type="checkbox" wire:model.live="customFilters.{{ $this->currentFilter->id }}" value="{{ $option->key }}">
            ...
            <div class="modal-footer">
                ...
                @if(isset($this->allFilters[end($modalHistory) + 1]))
