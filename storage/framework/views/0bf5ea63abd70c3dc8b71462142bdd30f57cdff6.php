  ...
  <div class="px-4 md:px-10 mx-auto w-full">
    ...
    <div class="bg-white p-5 rounded-md px-0 overflow-x-auto">
      ...
      <div class="text-sm font-bold mx-5 items-center bg-gray-100 flex justify-between md:flex-no-wrap flex-wrap pl-2 pr-2 mb-2 rounded">
        <?php if(auth()->user()->can('contact-us-delete-all')): ?>
        ...
        <table class="table-auto w-full text-sm">
          ...
          <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
            ...
            <tr>
              ...
              <td class="border-b px-4 py-2 text-center"><?php echo e($row->email); ?>

                <?php if($row->ad_link != ''): ?>
                <p class="text-sm">Ad link: <a class="underline" href="<?php echo e($row->ad_link); ?>" target="_blank"><?php echo e(Str::limit($row->ad_link, 20)); ?></a></p>
                <?php elseif($row->title != ''): ?>
                <p class="text-sm">Title: <?php echo e(Str::limit($row->title, 20)); ?></p>
                <?php endif; ?>
              </td>
              <td class="border-b px-4 py-2 text-center">
                ...
                <img src="<?php echo e($no_img); ?>" width="70" height="70" alt="attachment" />
                <?php else: ?>
                <img src="<?php echo e($imageUrl); ?>" width="70" height="70" alt="attachment" />
                <?php endif; ?>

              </td>
              <td class="border-b px-4 py-2 text-center"><?php echo e($row->phone); ?></td>
              <td class="border-b px-4 py-2 text-center"><?php echo e(Str::limit($row->description, 20)); ?></td>
              <td class="border-b px-4 py-2 text-center">
                <?php if(auth()->user()->can('contact-us-view')): ?>
                ...
                <?php endif; ?>
                <?php if(auth()->user()->can('contact-us-delete-single')): ?>
                <button wire:click="deleteReq('<?php echo e($row->id); ?>')" class="items-center px-2 py-1 bg-red-700 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-red-500 active:bg-red-900 focus:outline-none focus:border-red-900 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 ml-5"><i class="far fa-trash-alt"></i></a></button>
                <?php endif; ?>
              </td>
            </tr>
...
<script>
  ...
  $(document).on("click", "#view_report", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "<?php echo e(URL::to('view-description')); ?>",
      data: {
        id: id
      },
      ...
      if (allVals.length <= 0) {
        ...
        if (check == true) {
          ...
          $.ajax({
            type: 'POST',
            dataType: 'json',
            url: "<?php echo e(URL::to('delete-contact-us')); ?>",
            data: {
...
<?php endif; ?><?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/admin/contact-us/show.blade.php ENDPATH**/ ?>