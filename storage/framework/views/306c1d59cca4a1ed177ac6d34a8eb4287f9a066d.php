        ...
        <div @click="isMinimized = !isMinimized" class="flex items-center justify-between p-3 bg-gradient-to-r from-orange-500 to-orange-600 rounded-t-xl cursor-pointer">
            <div class="flex items-center">
                <div class="relative flex-shrink-0">
                    <img src="<?php echo e($recipient->profile_photo_path ? asset('storage/' . $recipient->profile_photo_path) : 'https://placehold.co/40x40'); ?>" 
                ...
                <div class="ml-3">
                    <p class="font-semibold text-sm"><?php echo e($recipient->name); ?></p>
        ...
        <div x-show="!isMinimized" x-transition class="flex flex-col" style="height: 70vh; max-height: 28rem;">
            ...
            <div id="chat-box-container" wire:poll.5s="loadMessages" class="flex-1 min-h-0 overflow-y-auto bg-gray-50">
                <div class="p-3 md:p-4 space-y-3 md:space-y-4 min-h-full flex flex-col justify-end">
                    <?php if($chatMessages && count($chatMessages) > 0): ?>
                        <?php $__currentLoopData = $chatMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php $isOutgoing = $message->from_id == auth()->id(); ?>
                            
                            <?php if($isOutgoing): ?>
                            
                            <div class="flex justify-end">
                                <div class="<?php echo e($message->make_offer ? 'bg-green-100 border border-green-200 text-green-800' : ($message->location ? 'bg-blue-100 border border-blue-200' : 'bg-orange-500 text-white')); ?> text-xs md:text-sm p-2 md:p-3 rounded-2xl rounded-br-lg max-w-[85%] md:max-w-[80%]">
                                    <?php if($message->location && $googleApiKey): ?>
                                        <div class="text-center">
                                            <h3 class="font-bold text-xs mb-2">📍 Shared Location</h3>
                                            <a href="https://www.google.com/maps?q=<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>" target="_blank" class="block hover:opacity-90 transition-opacity">
                                                <img src="https://maps.googleapis.com/maps/api/staticmap?center=<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>&zoom=15&size=300x200&markers=color:red%7C<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>&key=<?php echo e($googleApiKey); ?>" class="rounded-lg border border-gray-300">
                                            </a>
                                    ...
                                    <?php elseif($message->make_offer): ?>
                                        <div class="text-center">
                                            ...
                                            <p class="text-lg md:text-xl font-bold py-1"><?php echo e($currencySymbol); ?><?php echo e(number_format($message->msg)); ?></p>
                                            <?php if($message->accept_offer == 1): ?> 
                                                <span class="text-xs font-semibold text-white bg-green-500 px-2 py-0.5 rounded-full">Accepted</span>
                                            <?php elseif($message->denied_offer == 1): ?> 
                                    ...
                                    <?php elseif($message->attachment): ?> 
                                        <img 
                                            src="<?php echo e(asset('storage/' . $message->attachment)); ?>" 
                                            class="rounded-lg max-w-full h-auto cursor-pointer hover:opacity-90 transition-opacity"
                                            @click="showImageModal = true; currentImage = '<?php echo e(asset('storage/' . $message->attachment)); ?>'"
                                            alt="Sent image"
                                        >
                                    <?php else: ?> 
                            ...
                            <div class="flex justify-start">
                                <div class="<?php echo e($message->location ? 'bg-blue-100 border border-blue-200' : 'bg-white border border-gray-200 text-gray-800'); ?> text-xs md:text-sm p-2 md:p-3 rounded-2xl rounded-bl-lg max-w-[85%] md:max-w-[80%]">
                                    <?php if($message->location && $googleApiKey): ?>
                                        <div class="text-center">
                                            <h3 class="font-bold text-xs mb-2">📍 Shared Location</h3>
                                            <a href="https://www.google.com/maps?q=<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>" target="_blank" class="block hover:opacity-90 transition-opacity">
                                    ...
                                    <?php elseif($message->make_offer): ?>
                                        <div class="text-center">
                                            ...
                                            <?php if($message->accept_offer == 0 && $message->denied_offer == 0): ?>
                                            <div class="flex space-x-2 justify-center mt-2 border-t border-gray-300 pt-2">
                                                <button wire:click="acceptOffer('<?php echo e($message->id); ?>')" class="bg-green-500 text-white px-3 py-1 rounded-full text-xs hover:bg-green-600 transition-colors">
                                                    Accept
                                                </button>
                                                <button wire:click="rejectOffer('<?php echo e($message->id); ?>')" class="bg-red-500 text-white px-3 py-1 rounded-full text-xs hover:bg-red-600 transition-colors">
                                                    Reject
                                                </button>
                                            </div>
                                            <?php elseif($message->accept_offer == 1): ?>
                ...
                <div x-show="activeTab === 'offer'" x-transition>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex items-center gap-2 overflow-x-auto pb-2">
                            ...
                            <button 
                                wire:click="sendQuickOffer(<?php echo e($offer); ?>)" 
                    ...
                    <form wire:submit.prevent="sendOffer" class="flex items-center gap-2">
                        <div class="flex-1 min-w-0 relative">
                            <span class="absolute left-3 top-1/4 -translate-y-1/2 text-gray-500 font-medium text-sm"><?php echo e($currencySymbol); ?></span>
                            <input 
...
</div><?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/mini-chat.blade.php ENDPATH**/ ?>