...
class="flex flex-col h-full bg-white">
    ...
    <div class="flex items-center justify-between p-4 border-b bg-white flex-shrink-0 shadow-sm">
        ...
        <div class="flex items-center">
            ...
            <img src="<?php echo e($recipient->profile_photo_path ? asset('storage/' . $recipient->profile_photo_path) : 'https://placehold.co/40x40/E2E8F0/4A5568?text=' . strtoupper(substr($recipient->name, 0, 2))); ?>" 
                 class="h-10 w-10 rounded-full object-cover shadow-sm">
            <div class="ml-3">
                <p class="font-semibold text-gray-800"><?php echo e($recipient->name); ?></p>
                <p class="text-sm text-gray-500">RE: <?php echo e($post->title); ?></p>
            </div>
        </div>
        <?php endif; ?>
        <div class="relative">
    ...
    <?php if($chatMessages): ?>
        <?php $__empty_1 = true; $__currentLoopData = $chatMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php $isOutgoing = $message->from_id == auth()->id(); ?>
            
            
            <?php if($loop->first || \Carbon\Carbon::parse($message->created_at)->diffInHours(\Carbon\Carbon::parse($chatMessages[$loop->index-1]->created_at)) > 1): ?>
            <div class="flex justify-center my-4">
                <span class="bg-gray-100 text-gray-500 text-xs px-3 py-1 rounded-full">
                    <?php echo e(\Carbon\Carbon::parse($message->created_at)->format('M j, Y')); ?>
            ...
            <div class="flex items-start mb-4 group">
                <img src="<?php echo e($recipient->profile_photo_path ? asset('storage/' . $recipient->profile_photo_path) : 'https://placehold.co/32x32/E2E8F0/4A5568?text=' . strtoupper(substr($recipient->name, 0, 2))); ?>" 
                ...
                <div class="ml-3 max-w-xs lg:max-w-md">
                    <div class="bg-white text-gray-800 p-3 rounded-2xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md" 
                        ...
                        <?php if($message->msg && !$message->make_offer && !$message->location): ?>
                            <p class="text-sm leading-relaxed"><?php echo e($message->msg); ?></p>
                        
                        <?php elseif($message->attachment): ?>
                            <div class="relative">
                                <img @click="modalImageUrl = '<?php echo e($message->attachment ? asset('storage/' . $message->attachment) : ''); ?>'; showModal = true" 
                                     src="<?php echo e($message->attachment ? asset('storage/' . $message->attachment) : ''); ?>" 
                        ...
                        <?php elseif($message->location && $googleApiKey): ?>
                            <a href="https://maps.google.com/?q=<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>" target="_blank" class="block">
                                <div class="relative">
                                    <img src="https://maps.googleapis.com/maps/api/staticmap?center=<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>&zoom=15&size=300x200&markers=color:red%7C<?php echo e($message->latitude); ?>,<?php echo e($message->longitude); ?>&key=<?php echo e($googleApiKey); ?>" 
                        ...
                        <?php elseif($message->make_offer): ?>
                            <div class="p-2 text-center">
                                ...
                                <p class="text-2xl font-bold text-center py-1 text-gray-800">€<?php echo e(number_format($message->msg)); ?></p>
                                
                                
                                <?php if($message->make_offer && $message->accept_offer == 0 && $message->denied_offer == 0): ?>
                                <div class="mt-2 pt-2 border-t flex space-x-2">
                                    <button wire:click="acceptOffer('<?php echo e($message->id); ?>')" class="text-xs font-semibold bg-green-100 text-green-700 px-3 py-1 rounded-full hover:bg-green-200">Accept</button>
                                    <button wire:click="denyOffer('<?php echo e($message->id); ?>')" class="text-xs font-semibold bg-red-100 text-red-700 px-3 py-1 rounded-full hover:bg-red-200">Deny</button>
                                </div>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="text-xs text-gray-400 mt-1 ml-1"><?php echo e(\Carbon\Carbon::parse($message->created_at)->format('h:i a')); ?></div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="flex items-start justify-end mb-4 group">
                <div class="max-w-xs lg:max-w-md">
                    <div class="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-3 rounded-2xl shadow-sm transition-all duration-200 hover:shadow-md" 
                        ...
                        <?php elseif($message->make_offer): ?>
                            <div class="p-2 text-center">
                                ...
                                <p class="text-2xl font-bold text-center py-1">€<?php echo e(number_format($message->msg)); ?></p>
                                 
                                <?php if($message->make_offer && $message->accept_offer == 0 && $message->denied_offer == 0): ?>
                                 <!--<div class="mt-2 pt-2 border-t border-white/20">
                                    <button @click="activeTab = 'offer'; $wire.set('offerAmount', <?php echo e($message->msg); ?>)" class="text-xs font-semibold bg-white/20 text-white px-3 py-1 rounded-full hover:bg-white/30">Edit Offer</button>
                                </div>!-->
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="text-xs text-gray-400 mt-1 mr-1 text-right flex items-center justify-end">
                        <span><?php echo e(\Carbon\Carbon::parse($message->created_at)->format('h:i a')); ?></span>
                ...
                <img src="<?php echo e(auth()->user()->profile_photo_path ? asset('storage/' . auth()->user()->profile_photo_path) : 'https://placehold.co/32x32/E2E8F0/4A5568?text=' . strtoupper(substr(auth()->user()->name, 0, 2))); ?>" 
    ...
    <div class="p-4 border-t bg-white flex-shrink-0 shadow-lg">
      	<?php if($isBlocked): ?>
            <div class="text-center p-4 bg-gray-100 rounded-lg">
                <p class="text-sm font-medium text-gray-600">
                    <?php if(App\Models\TblBlockeduser::where('blocked_by', auth()->id())->where('blocked_id', $recipient->id)->exists()): ?>
        ...
        <div x-show="activeTab === 'message'" x-transition>
            ...
            <form wire:submit.prevent="sendMessage" class="flex items-center h-16 rounded-full bg-gray-100 px-4 border border-transparent focus-within:border-orange-300 focus-within:bg-white transition-all duration-200">
                <div class="flex items-center">
                    <label for="image-upload-<?php echo e($conversationData['id']); ?>" class="cursor-pointer p-2 text-gray-500 hover:text-orange-600 transition-colors" title="Attach image">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    </label>
                    <input id="image-upload-<?php echo e($conversationData['id']); ?>" type="file" wire:model="image" class="hidden" >
        ...
        <div x-show="activeTab === 'offer'" x-transition style="display: none;">
          	<?php if(!empty($suggestedOffers)): ?>
            <div class="flex items-center space-x-2 mb-3 overflow-x-auto pb-2">
                <?php $__currentLoopData = $suggestedOffers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $offer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> 
                    <button wire:click="sendQuickOffer(<?php echo e($offer); ?>)" class="text-sm text-orange-700 bg-orange-100 hover:bg-orange-200 py-1.5 px-4 rounded-full flex-shrink-0 transition-colors mr-2">
                        <?php echo e($currencySymbol); ?><?php echo e(number_format($offer)); ?>
...
</div><?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/chat-box.blade.php ENDPATH**/ ?>