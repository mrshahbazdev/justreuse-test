<div>
            ...
            <div class=" rounded-lg shadow-sm p-4 border border-gray-200 mb-6 flex flex-col md:flex-row items-center gap-4">
                ...
                <div class="flex items-center">
                    ...
                    <button wire:click="confirmBulkDelete" wire:loading.attr="disabled"
                        ...
                        Delete (<?php echo e(count($selectedAds)); ?>)
                    </button>
                    <?php endif; ?>
                </div>
                        ...
                        <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative">
                                ...
                                <div class="absolute top-3 left-3 z-10">
                                    <input wire:model="selectedAds" value="<?php echo e($row->fav_id); ?>" type="checkbox" class="h-5 w-5 rounded text-green-600 focus:ring-green-500 border-gray-300 shadow-sm">
                                </div>

                                <!-- Image Section -->
                                <a href="<?php echo e($slug); ?>" target="_blank" class="block">
                                    <img src="<?php echo e($imgUrlfinal); ?>" alt="<?php echo e($row->title); ?>" class="w-full h-48 object-cover rounded-t-xl">
                                </a>

                                <!-- Card Content -->
                                <div class="p-4 flex flex-col flex-grow">
                                    <a href="<?php echo e($slug); ?>" target="_blank" class="flex-grow">
                                        <h3 class="font-bold text-slate-900 line-clamp-2 ad-title capitalize hover:text-green-600"><?php echo e($row->title); ?></h3>
                                    </a>

                                    
                                    <p class="text-green-600 font-extrabold text-lg mt-2 ad-price"><?php echo $currency_symbol[0] ?? ''; ?><?php echo e(number_format($row->price, 2)); ?></p>

                                    <div class="mt-3 text-xs text-slate-500 flex items-center justify-between">
                                        <span class="ad-loc truncate pr-2"><i class="fa fa-map-marker mr-1"></i><?php echo e(!empty($row->locality) ? $row->locality : $row->city_name); ?></span>
                                        <span class="flex-shrink-0"><?php echo e(\Carbon\Carbon::parse($row->created_at)->format('d M Y')); ?></span>
                                    </div>
                                    
                                    <button wire:click="confirmDelete('<?php echo e($row->fav_id); ?>')" wire:loading.attr="disabled"
    ...
    <div x-data="{ show: <?php if ((object) ('showDeleteConfirmation') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('showDeleteConfirmation'->value()); ?>')<?php echo e('showDeleteConfirmation'->hasModifier('defer') ? '.defer' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('showDeleteConfirmation'); ?>')<?php endif; ?> }" x-show="show" @keydown.escape.window="show = false" class="fixed z-50 inset-0 overflow-y-auto" style="display: none;">
    ...
    <div x-data="{ show: <?php if ((object) ('showBulkDeleteConfirmation') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('showBulkDeleteConfirmation'->value()); ?>')<?php echo e('showBulkDeleteConfirmation'->hasModifier('defer') ? '.defer' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('showBulkDeleteConfirmation'); ?>')<?php endif; ?> }" x-show="show" @keydown.escape.window="show = false" class="fixed z-50 inset-0 overflow-y-auto" style="display: none;">
         <div class="flex items-center justify-center min-h-screen">
            <div x-show="show" x-transition.opacity class="fixed inset-0 transition-opacity" aria-hidden="true"><div class="absolute inset-0 bg-gray-500 opacity-75"></div></div>
            <div x-show="show" x-transition class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2"><p class="text-sm text-gray-500">Are you sure you want to remove the selected <?php echo e(count($selectedAds)); ?> ads? This action cannot be undone.</p></div>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
...
<?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/favourite-ads.blade.php ENDPATH**/ ?>