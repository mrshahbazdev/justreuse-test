                ...
                <table class="table-auto w-full">
                    ...
                    <tbody class="border border-b-0 border-l-0 border-r-0 tablelast">
                        ...
                        <tr>
                            ...
                            ?>
                            <td class="border-b px-4 py-2 text-center">
                                <?php if(auth()->user()->can('banner-ads-approve')): ?>
                                <button data-created="<?php echo date('Y-m-d', strtotime($BannerAd->created_at)); ?>" data-id="<?php echo e($BannerAd->id); ?>" class="approve items-center px-2 py-1 bg-green-500 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-green-700 focus:outline-none focus:border-green-900 focus:shadow-outline-green disabled:opacity-25 transition ease-in-out duration-150">Approve</button>
                                <?php endif; ?>

                                <?php if(auth()->user()->can('banner-ads-refund')): ?>
                                <button data-id="<?php echo e($BannerAd->id); ?>" data-pay="<?php echo e($pay); ?>" class="refund items-center px-2 py-1 bg-red-500 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-red-700 focus:outline-none focus:border-red-900 focus:shadow-outline-red disabled:opacity-25 transition ease-in-out duration-150 ml-5">Refund</button>
                                <?php endif; ?>

                                <?php if(auth()->user()->can('banner-ads-view')): ?>
                                <button class="items-center px-2 py-1 bg-green-500 border border-transparent rounded-md text-xs text-white tracking-widest hover:bg-orange-500 focus:outline-none focus:border-indigo-900 focus:shadow-outline-indigo disabled:opacity-25 transition ease-in-out duration-150 ml-5" wire:click="view('<?php echo e($BannerAd->id); ?>')"><i class="fas fa-eye"></i> View</button>
                                <?php endif; ?>
                            </td>
                            <?php else: ?>
                            <td class="border-b px-4 py-2 text-center">
    ...
    <script>
        ...
        function approve_banner_ads(id, date) {
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: "<?php echo e(URL::to('approve-banner-ads')); ?>",
                data: {
                    id: id,
                    update_date: date,
                },
        ...
        function p_refund(id, type) {
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: "<?php echo e(URL::to('refund_payment_paypal')); ?>",
                data: {
                    id: id,
                    type: type
                },
...
</div><?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/admin/banner_advertisements/show.blade.php ENDPATH**/ ?>