    ...
    <div>
      <div class="flex flex-wrap items-center bg-gray-100 p-2 py-3 mb-2 mx-2 xl:mx-5">
        ...
        <div class="relative w-full max-w-full flex-grow flex-1 text-right">
          ...
          <button class="bg-green-500 text-white text-sm px-4 py-2 rounded-sm shadow hover:shadow-md outline-none focus:outline-none hover:bg-orange-500"><a href="<?php echo e(URL::to($addnew_url)); ?>"  class="capitalize"><i class="fa fa-plus"></i>
              <?php echo e($addnew_title); ?></a></button>
          <?php endif; ?>
          <?php if(auth()->user()->can('category-reorder')): ?>
          <button class="bg-green-500 text-white text-sm px-4 py-2 rounded-sm shadow hover:shadow-md outline-none focus:outline-none hover:bg-orange-500"><a href="<?php echo e(URL::to($reorder_url)); ?>" class="capitalize"><i class="fa fa-bars"></i>
              <?php echo e($reorder_title); ?></a></button>
          <?php endif; ?>
        </div>
      </div>
        ...
        <table class="mt-5 border-collapse border-t border-gray-300 min-w-full">
          <thead>
            <tr class="">
              ...
              <?php endif; ?>
              <?php if(auth()->user()->can('category-custom-fields-edit')): ?>
              ...
              <?php if(auth()->user()->can('category-edit') || auth()->user()->can('category-delete')): ?>
          ...
          <tbody class="border-t grid-style tablelast">
            ...
            <tr>
              <td class="p-2 border-b border-gray-300 text-center"><?php echo e($i); ?></td>
              <td class="p-2 border-b border-gray-300 text-center"><?php echo e($row->title); ?></td>
              <?php if(auth()->user()->can('category-edit')): ?>
              <td class="p-2 border-b border-gray-300 text-center">                
                <button class="bg-gray-300 text-xs px-1.5 py-1 rounded-lg bg-gradient-to-r hover:bg-orange-500 hover:text-white"><a href="<?php echo e(URL::to('admin/category/'.$row->uuid.'/subcategories')); ?>"><?php echo e($row->descendantsOf($row->id)->toTree($row->id)->count()); ?> subcategories
                  </a></button>
              </td>
              <?php endif; ?>
              <?php if(auth()->user()->can('category-custom-fields-edit')): ?>
              ...
              <?php if(is_null($row->parent_id)){ ?>
                  -
                <?php } else {?>
                <button class="bg-gray-300 text-xs px-1.5 py-1 rounded-lg hover:bg-orange-500 hover:text-white"><a href="<?php echo e(URL::to('admin/custom-fields?id='.$row->uuid)); ?>"><?php echo e($fieldcount); ?> custom fields</a></button>
                <?php } ?>  
              </td>
              <?php endif; ?>
...
<script>
  ...
  $(document).on("click", ".toggle_showhome", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "<?php echo e(URL::to('show-home-category')); ?>",
      data: {
  ...
  $(document).on("click", ".en_dis", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "<?php echo e(URL::to('enable-disable-category')); ?>",
      data: {
  ...
  $(document).on("click", ".buynow", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "<?php echo e(URL::to('buynow-enable-disable-category')); ?>",
      data: {
  ...
  $(document).on("click", ".exchange", function(e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: "<?php echo e(URL::to('exchange-enable-disable-category')); ?>",
      data: {
...
<?php endif; ?><?php /**PATH /home/justreused/htdocs/www.justreused.com/resources/views/livewire/admin/category/show.blade.php ENDPATH**/ ?>